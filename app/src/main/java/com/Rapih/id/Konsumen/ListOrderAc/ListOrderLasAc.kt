...
import com.Rapih.id.Konsumen.DetailOrderAC.DetailOrderLasAcKonsumen
...
class ListOrderLasAc :AppCompatActivity(){
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.list_las_ac)

        Rak.initialize(this)

        urlac = "http://rapih.id/api/las_ac/listorderlasackonsumen.php?id_konsumen_ac= " + Rak.grab("idkonsumen")

        Log.d("TAG", urlac)

        lstHistoriac = findViewById(R.id.rvListOrderLasAc) as RecyclerView

        pglistac = findViewById(R.id.progressBarLasAc) as ProgressBar

        swipeRefreshLayout = findViewById(R.id.swipeorderlasac)
                ...
                try {
                    ...
                    adapter!!.setListener(object: ItemClickListener<OrderKonsumenAc> {
                        override fun onClicked(OrderKonsumenAc: OrderKonsumenAc?, position: Int, view: View?) {
                            ...
                            val intent = Intent(this@ListOrderLasAc, DetailOrderLasAcKonsumen::class.java)
